...
class UsersManager {
    ...
    // Authentication
    public function getTimeConnected(User $user) {
        $req = $this->db->prepare("SELECT COUNT(*) as total FROM logAuthen WHERE login = ? AND isSuccess = 'true'");
        $req->execute(array(
            $user->get("username")
        ));

        while ($data = $req->fetch()) {
            $timeConnected = $data["total"];
        }

        return $timeConnected;
    }

    public function getLastConnection(User $user) {
        $req = $this->db->prepare("SELECT * FROM logAuthen WHERE login = ? AND isSuccess = 'true' ORDER BY date DESC LIMIT 1");
        $req->execute(array(
            $user->get("username")
        ));

        while ($data = $req->fetch()) {
            $lastConnection = $data["date"];
        }

        return (isset($lastConnection)) ? $lastConnection : "Aucune";
    }
